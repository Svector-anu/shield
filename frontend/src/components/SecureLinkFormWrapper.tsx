...
const SecureLinkForm = () => {
  ...
  useEffect(() => {
    ...
    faceApiWorker.current.onmessage = (event) => {
      ...
      switch (type) {
        case 'MODELS_LOADED':
          setFeedbackMessage('AI models loaded. You can now select a face image.');
          break;
        case 'FACE_DETECTED':
          setFaceDescriptor(payload);
        ...
        case 'NO_FACE_DETECTED':
          ...
          break;
        case 'ERROR':
          setFeedbackMessage(`Error: ${payload}`);
          break;
        default:
          break;
      }
  ...
  const handleRecipientImagesChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      setFeedbackMessage('Processing face in the background...');
      // Send the image blob to the worker for processing.
      faceApiWorker.current?.postMessage({ type: 'DETECT_FACE', payload: { imageBlob: image } });
    }
  };
